...
from constants import DT, XML_DIR, START_ARM_POSE, SINGLE_START_ARM_POSE
...
def make_sim_env(task_name):
    ...
    if 'sim_transfer_cube' in task_name:
        ...
        xml_path = os.path.join(XML_DIR, f'bimanual_viperx_lift_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = LiftCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)
...
class BimanualViperXTask(base.Task):
    ...
    def get_observation(self, physics, subscriber_instance):
        ...
        if subscriber_instance != None and subscriber_instance.head_pos_rotating == True:
...
class LiftCubeTask(SingleViperXTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 4
    ...
    def initialize_episode(self, physics):
        ...
        # reset qpos, control and box position
        with physics.reset_context():
            physics.named.data.qpos[:8] = SINGLE_START_ARM_POSE
            np.copyto(physics.data.ctrl, SINGLE_START_ARM_POSE)
            assert BOX_POSE[0] is not None
            physics.named.data.qpos[-7:] = BOX_POSE[0]
            # print(f"{BOX_POSE=}")
        super().initialize_episode(physics)
